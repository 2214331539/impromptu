...
export function ClassesPage() {
  ...
  const activeClassId = selectedId ?? classes.data?.[0]?.id ?? null;
  const members = useQuery({ queryKey: ["class-members", activeClassId], queryFn: () => api<Member[]>(`/classes/${activeClassId}/members`), enabled: !!activeClassId });
  ...
  const remove = useMutation({ mutationFn: (studentId: number) => api<void>(`/classes/${activeClassId}/members/${studentId}`, { method: "DELETE" }), onSuccess: () => client.invalidateQueries({ queryKey: ["class-members", activeClassId] }) });
  ...
  const selected = classes.data?.find((item) => item.id === activeClassId);
  return <div className="page-enter"><header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="page-title">班级管理</h1><p className="mt-2 text-sm text-muted">管理学生名单与班级邀请码。</p></div><Button icon={<Plus className="h-4 w-4" />} onClick={() => setOpen(true)}>创建班级</Button></header>
    {!classes.data?.length ? <EmptyState title="还没有班级" description="创建班级后，将邀请码发给学生即可加入。" action={<Button icon={<Plus className="h-4 w-4" />} onClick={() => setOpen(true)}>创建班级</Button>} /> : <div className="grid gap-5 lg:grid-cols-[280px_1fr]"><aside className="surface scrollbar-none flex h-fit snap-x gap-1 overflow-x-auto p-2 lg:block lg:overflow-visible">{classes.data.map((item) => <button key={item.id} onClick={() => setSelectedId(item.id)} className={`w-56 shrink-0 snap-start rounded-[12px] p-3 text-left transition lg:w-full ${activeClassId === item.id ? "bg-black/[.055]" : "hover:bg-black/[.025]"}`}><p className="truncate text-sm font-medium">{item.name}</p><p className="mt-1 text-xs text-muted">{item.student_count} 名学生 · {item.task_count} 项任务</p></button>)}</aside>
