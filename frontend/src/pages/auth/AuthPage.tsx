...
const baseSchema = z.object({ student_no: z.string().min(1, "请输入账号"), password: z.string().min(6, "密码至少 6 位"), name: z.string().optional() });
...
export function AuthPage({ mode }: { mode: "login" | "register" }) {
  ...
  const schema = baseSchema.superRefine((values, context) => {
    ...
    if (mode === "register" && !sixDigitStudentId.test(values.student_no)) context.addIssue({ code: "custom", path: ["student_no"], message: "学号必须为 6 位数字" });
    if (mode === "login" && !sixDigitStudentId.test(values.student_no) && !staffId.test(values.student_no)) context.addIssue({ code: "custom", path: ["student_no"], message: "学生请输入 6 位数字学号，教师请输入工号" });
  });
  ...
  return <div className="auth-canvas flex min-h-screen items-center px-4 py-8 sm:px-6 sm:py-12">
    <div className="auth-shell mx-auto grid w-full max-w-[1060px] overflow-hidden rounded-[22px] bg-white lg:min-h-[660px] lg:grid-cols-[.92fr_1fr]">
      <main className="flex items-center justify-center p-6 sm:p-10 lg:order-2 lg:p-14">
        <div className="page-enter w-full max-w-[400px]">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full border border-black/10 bg-[#f5f5f7] px-3 py-1 text-xs font-medium text-muted">学生口语角</span>
          </div>
          <div className="mt-10">
          ...
          <form className="mt-8 space-y-4" onSubmit={form.handleSubmit((data) => mutation.mutate(data))}>
            {mode === "register" && <div><label className="label">姓名</label><input className="field" autoComplete="name" placeholder="你的姓名" {...form.register("name")} />{form.formState.errors.name && <p className="mt-1 text-xs text-danger">{form.formState.errors.name.message}</p>}</div>}
      ...
      <aside className="doodle-panel hidden items-center justify-center overflow-hidden p-12 lg:order-1 lg:flex">
        <div className="relative z-10 flex max-w-[350px] flex-col items-center text-center">
          <div className="speech-note mb-10 self-start rounded-[16px] bg-white px-5 py-3 text-left text-sm font-medium leading-6 text-ink"><MessageCircle className="mr-2 inline h-4 w-4 text-accent" />别怕停顿，先把想法说出来。</div>
          ...
          <div className="mt-7 flex items-center gap-2 text-xs font-medium"><span className="rounded-full border border-black/10 bg-white px-3 py-1.5">清晰</span><span className="rounded-full border border-black/10 bg-white px-3 py-1.5">自信</span><span className="rounded-full border border-black/10 bg-white px-3 py-1.5">真诚</span><Star className="ml-1 h-4 w-4 text-muted" /></div>
        </div>
      </aside>
    </div>
  </div>;
