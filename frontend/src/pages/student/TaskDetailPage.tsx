...
import { ArrowLeft, CalendarClock, Check, Clock3, Dices, FileText, RefreshCw } from "lucide-react";
...
export function TaskDetailPage() {
  ...
  return <div className="page-enter mx-auto max-w-4xl">
    ...
    <article className="surface overflow-hidden"><div className="border-b border-black/[.07] p-6 sm:p-8"><div className="flex flex-wrap items-center gap-2"><Badge tone={task.my_phase === "submitted" ? "green" : "blue"}>{task.my_phase ? phaseLabel[task.my_phase] : "待开始"}</Badge><span className="text-xs text-muted">{task.class_name}</span></div><h1 className="mt-5 text-[28px] font-semibold leading-tight sm:text-[36px]">{task.name}</h1><p className="mt-4 max-w-2xl whitespace-pre-line text-[15px] leading-7 text-muted">{task.description || "完成本次随机口语训练。"}</p></div>
      <div className="grid gap-px bg-black/[.06] sm:grid-cols-4"><Info icon={<Clock3 />} label="准备时间" value={formatDuration(task.preparation_seconds)} /><Info icon={<Clock3 />} label="演讲时间" value={formatDuration(task.speaking_seconds)} /><Info icon={<RefreshCw />} label="重新抽题" value={`${task.redraw_limit} 次`} /><Info icon={<CalendarClock />} label="截止时间" value={formatDate(task.due_at)} /></div>
      <div className="p-6 sm:p-8"><h2 className="section-title">训练规则</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><Rule text="题目由系统从已启用题目中随机抽取" /><Rule text={`确认题目后开始 ${formatDuration(task.preparation_seconds)} 准备倒计时`} /><Rule text="演讲阶段需要允许浏览器使用麦克风" /><Rule text="完成演讲后上传录音即可提交" /></div>
        {start.error && <p className="mt-5 text-sm text-danger">{start.error.message}</p>}
        <div className="mt-8 flex flex-wrap items-center gap-3"><Button size="lg" loading={start.isPending} disabled={expired || task.my_phase === "submitted"} icon={<Dices className="h-4 w-4" />} onClick={() => task.my_session_id ? navigate(`/app/training/${task.my_session_id}`) : start.mutate()}>{task.my_phase && task.my_phase !== "submitted" ? "继续训练" : task.my_phase === "submitted" ? "训练已提交" : expired ? "任务已截止" : "开始随机抽题"}</Button>{task.my_phase === "submitted" && task.my_session_id && <Button variant="secondary" size="lg" icon={<FileText className="h-4 w-4" />} onClick={() => navigate(`/app/history/${task.my_session_id}`)}>查看结果</Button>}</div>
      </div>
    </article>
  </div>;
}
