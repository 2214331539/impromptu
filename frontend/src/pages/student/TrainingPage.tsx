...
export function TrainingPage() {
  ...
  return <TrainingPageContent key={sessionId || "new"} />;
}
...
function TrainingPageContent() {
  ...
  useEffect(() => {
    ...
    if (session.phase === "submitted") { void recordingCache(audioKey, "delete").catch(() => undefined); forgetTask(`/app/training/${id}`); return; }
    rememberTask(`/app/training/${id}`, session.task.name);
    void recordingCache(audioKey, "read").then((cached) => {
      if (!cached || cached.attempt !== session.recording_attempts_started || session.recordings.some((r) => r.attempt_number === cached.attempt)) return;
      setPendingBlob(cached);
      setLocalAudio(URL.createObjectURL(cached.blob));
