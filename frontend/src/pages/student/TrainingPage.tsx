...
import { Check, ChevronRight, Dices, Mic2, RefreshCw, Save, Send, ShieldCheck, Square, UploadCloud } from "lucide-react";
...
export function TrainingPage() {
  ...
  const query = useQuery({ queryKey: ["session", id], queryFn: () => api<TrainingSession>(`/sessions/${id}`), enabled: Number.isFinite(id), refetchInterval: (q) => q.state.data?.phase === "preparing" ? 5000 : false });
  ...
  return <div className="page-enter mx-auto max-w-5xl">
    ...
    {session.phase === "preparing" && <PreparationStage session={session} onRefresh={refresh} />}
...
function TrainingSteps({ phase }: { phase: TrainingSession["phase"] }) {
  const phases = ["drawing", "mic_check", "preparing", "speaking", "review"] as const;
  const current = phase === "submitted" ? 4 : phase === "mic_check" ? 1 : Math.max(0, phases.indexOf(phase) - 1);
  return <div className="mb-7 flex items-center justify-center gap-1.5 sm:gap-3">{["抽题", "试音", "准备", "演讲", "提交"].map((label, index) => <div key={label} className="flex items-center gap-1.5 sm:gap-3"><span className={`grid h-7 min-w-7 place-items-center rounded-full px-2 text-[12px] font-medium ${index <= current ? "bg-ink text-white" : "bg-black/[.05] text-muted"}`}>{index < current ? <Check className="h-3.5 w-3.5" /> : label}</span>{index < 4 && <ChevronRight className="h-3.5 w-3.5 text-black/20" />}</div>)}</div>;
}
...
function PreparationStage({ session, onRefresh }: { session: TrainingSession; onRefresh: () => Promise<void> }) {
  ...
  const onExpire = useCallback(() => { playTimerTone(); start.mutate(); }, [start]);
  const remaining = useCountdown(session.preparation_ends_at, session.server_time, onExpire);
  return <section className="grid gap-5 lg:grid-cols-[.9fr_1.1fr]"><div className="surface flex flex-col p-6 sm:p-8"><div><Badge tone="blue">准备阶段</Badge><h1 className="mt-4 text-xl font-semibold leading-8">{session.final_topic?.prompt}</h1></div><div className="my-auto py-7"><CountdownRing remaining={remaining} total={session.task.preparation_seconds} label="准备剩余" /></div><div className="flex justify-center"><Button disabled={!session.task.allow_early_finish} loading={start.isPending} onClick={() => start.mutate()}>提前结束准备</Button></div>{start.error && <p className="mt-3 text-center text-sm text-danger">{start.error.message}</p>}</div><div className="surface flex min-h-[540px] flex-col p-6 sm:p-8"><div className="flex items-center justify-between"><div><h2 className="section-title">准备笔记</h2><p className="mt-1 text-sm text-muted">记录关键词和结构，不必写成完整稿件。</p></div><span className={`flex items-center gap-1 text-xs ${saveState === "error" ? "text-danger" : "text-muted"}`}><Save className="h-3.5 w-3.5" />{saveState === "saving" ? "保存中" : saveState === "error" ? "保存失败" : "已保存"}</span></div><textarea value={draft ?? session.note} onChange={(event) => setDraft(session.id, event.target.value)} className="mt-5 min-h-[390px] flex-1 resize-none rounded-[14px] border border-black/[.08] bg-[#fafafa] p-4 text-[15px] leading-7 outline-none focus:border-accent/50" placeholder="观点…\n论据…\n例子…\n结尾…" /></div></section>;
}
...
function SpeakingStage({ session, recorder, onFinish, onRefresh }: { session: TrainingSession; recorder: Recorder; onFinish: () => Promise<void>; onRefresh: () => Promise<void> }) {
  ...
  return <section className="surface mx-auto max-w-3xl overflow-hidden"><div className="border-b border-black/[.06] px-6 py-5 text-center"><Badge tone="red">演讲阶段</Badge><h1 className="mx-auto mt-3 max-w-2xl text-lg font-semibold leading-7">{session.final_topic?.prompt}</h1></div><div className="p-7 text-center sm:p-10"><CountdownRing remaining={remaining} total={session.task.speaking_seconds} label="演讲剩余" /><div className="mx-auto mt-5 max-w-md"><div className="flex h-12 items-center gap-1 rounded-[12px] bg-black/[.035] px-4" aria-label="实时音量">{Array.from({ length: 24 }, (_, i) => <span key={i} className={`h-1 flex-1 rounded-full transition-all duration-100 ${i / 24 < recorder.volume ? "bg-accent" : "bg-black/10"}`} style={{ transform: `scaleY(${i / 24 < recorder.volume ? 1 + recorder.volume * 4 : 1})` }} />)}</div><p className={`mt-3 flex items-center justify-center gap-2 text-sm ${recorder.recording ? "text-danger" : "text-muted"}`}><span className={`h-2 w-2 rounded-full ${recorder.recording ? "animate-pulse bg-danger" : "bg-black/20"}`} />{recorder.recording ? "正在录音" : recorder.permission === "denied" ? "麦克风授权失败" : "等待开始录音"}</p></div>{begin.error && <InlineMessage>{begin.error.message}</InlineMessage>}<div className="mt-6 flex justify-center">{recorder.recording ? <Button variant="danger" size="lg" icon={<Square className="h-4 w-4 fill-current" />} onClick={() => void onFinish()}>结束演讲</Button> : <Button size="lg" icon={<Mic2 className="h-4 w-4" />} loading={begin.isPending} onClick={() => begin.mutate()}>开始演讲并录音</Button>}</div><p className="mt-4 text-xs text-muted">倒计时结束时，录音会自动停止。</p></div></section>;
}
