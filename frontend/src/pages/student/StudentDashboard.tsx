...
import type { Dashboard, WritingAssignment } from "../../types";
...
export function StudentDashboard() {
  ...
  const writing = useQuery({ queryKey: ["writing-assignments"], queryFn: () => api<WritingAssignment[]>("/writing/assignments") });
  ...
  return <div className="page-enter space-y-10">
    ...
    <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      ...
      <Metric icon={<ClipboardList />} label="待完成口语任务" value={metrics.pending || 0} suffix="项" />
    </section>
    <section><div className="mb-4 flex items-center justify-between"><h2 className="section-title">待完成口语任务</h2><Link className="flex items-center gap-1 text-sm text-accent" to="/app/tasks">全部口语任务<ArrowRight className="h-3.5 w-3.5" /></Link></div>{tasks.length ? <div className="grid gap-4 lg:grid-cols-2">{tasks.slice(0, 4).map((task) => <TaskCard key={task.id} task={task} />)}</div> : <EmptyState title="任务已经完成" description="当前没有待完成的口语训练。" />}</section>
    <section><div className="mb-4 flex items-center justify-between"><h2 className="section-title">写作任务</h2><Link className="text-sm text-accent" to="/app/writing">全部写作任务</Link></div>{writing.isLoading ? <LoadingState /> : writing.isError ? <ErrorState message={writing.error.message} retry={() => writing.refetch()} /> : writing.data?.some((item) => item.my_submission_status !== "finalized") ? <div className="grid gap-4 lg:grid-cols-2">{(writing.data || []).filter((item) => item.my_submission_status !== "finalized").slice(0, 4).map((item) => <Link className="surface p-5" key={item.id} to={item.my_submission_id ? `/app/writing/session/${item.my_submission_id}` : `/app/writing/${item.id}`}><div className="flex items-center justify-between text-xs text-muted"><span>{item.class_name}</span><span>{item.status === "closed" ? "已关闭" : `截止 ${formatDate(item.due_at)}`}</span></div><h3 className="mt-3 font-semibold">{item.title}</h3><p className="mt-2 text-sm text-accent">{item.my_submission_id ? "继续写作" : "查看任务"}</p></Link>)}</div> : <EmptyState title="暂无写作任务" description="加入班级后，教师发布的写作任务会显示在这里。" />}</section>
