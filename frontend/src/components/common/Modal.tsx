...
export function Modal({ open, onClose, title, children, size = "md" }: { open: boolean; onClose: () => void; title: string; children: ReactNode; size?: "md" | "lg" }) {
  if (!open) return null;
  const width = size === "lg" ? "sm:max-w-4xl" : "sm:max-w-lg";
  return <div className="fixed inset-0 z-50 grid overflow-y-auto overscroll-contain bg-black/25 p-3 backdrop-blur-[2px] sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-label={title}>
    <div className={`my-auto max-h-[calc(100dvh-1.5rem)] w-full overflow-y-auto rounded-[18px] border border-black/10 bg-[#fbfbfc] p-4 shadow-2xl sm:max-h-[calc(100dvh-2rem)] sm:rounded-[20px] sm:p-5 ${width}`}>
