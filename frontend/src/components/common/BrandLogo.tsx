...
export function BrandLogo({ size = "md", showName = false, className = "" }: Props) {
  return <span className={`inline-flex items-center gap-2.5 ${className}`}>
    <span className={`${sizes[size]} shrink-0 overflow-hidden bg-white`}>
      <img width={imageSizes[size]} height={imageSizes[size]} className="h-full w-full scale-[1.38] object-cover object-center mix-blend-multiply" src={logoUrl} alt="声场 Logo" />
    </span>
    {showName && <span className="text-[17px] font-semibold text-ink">声场</span>}
  </span>;
}
